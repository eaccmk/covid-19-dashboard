...
module('Unit | Service | elide', function (hooks) {
  ...
  test('_buildFetchUrl - gt, lt, ge & le', function (assert) {
    ...
    const actualUrl = service._buildUrl('healthRecords', {
      ...
      l3: { referenceDate: '2020-04-30T00:00Z' }
    });
    assert.equal(
      actualUrl.toString(),
      'https://covid19.knowledge.yahoo.com/api/json/v1/healthRecords?filter=referenceDate%3Dlt%3D2020-05-01T00%3A00Z%3BreferenceDate%3Dgt%3D2020-04-03T00%3A00Z%3BreferenceDate%3Dge%3D2020-04-04T00%3A00Z&page%5Boffset%5D=0&page%5Blimit%5D=4000',
      '_buildSearchUrl builds a url given a request'
    );
  });
