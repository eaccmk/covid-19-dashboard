...
export default class CaseDetailsComponent extends Component {
  ...
  @(task(function* (location) {
    this.loading = true;
    const { data: healthRecord } = yield this.elide.fetch.linked().perform('latestHealthRecords', {
      eq: { 'place.id': location.id },
      fields: {
        latestHealthRecords: ['totalConfirmedCases', 'totalDeaths', 'dataSource', 'referenceDate'],
      },
      limit: 1,
    });
