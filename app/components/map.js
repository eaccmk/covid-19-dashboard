...
export default class MapComponent extends Component {
  ...
  @task(function* () {
    ...
    if (![SUPERNAME_TYPE, COUNTRY_TYPE].includes(locationPlaceType)) {
      ...
      if (data.data) {
        this.locationRecords = data.data.filter((r) => r.attributes[this.args.fieldToShow]);
      }
    }
  })
  ...
  @task(function* () {
    ...
    const { stateAdminAreas: stateAdminAreaRecords, countries: countryRecords } = yield hash({
      stateAdminAreas,
      countries,
    });
